        ...
        for persona_conversation_data in json_data:
            ...
            for dialogue_turn in persona_conversation_data["dlg_turns"]:
                cur_conversation.append(
                    {"role": "user",
                        "content": dialogue_turn["user_utterance"]}
                )
                cur_conversation.append(
                    {
                        "role": "assistant",
...
def _display_references(citation_dict):
    if citation_dict:
        reference_list = [f"reference [{i}]" for i in range(
...
def set_storm_runner():
    ...
    llm_configs.init_openai_model(
        openai_api_key=st.secrets["OPENAI_API_KEY"], openai_type="openai", azure_api_key=st.secrets["AZURE_API_KEY"]
    )
    ...
    rm = YouRM(
        ydc_api_key=st.secrets["YDC_API_KEY"], k=engine_args.search_top_k)
